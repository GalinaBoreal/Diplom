...
class RegisterAccount(APIView):
    ...
    # Пример простейшего описания для автогенерации описания метода
    @extend_schema(description='Process a POST request and create a new user.', methods=["POST"])
    def post(self, request, *args, **kwargs):
        """
            ...
            """
        # проверяем обязательные аргументы методом issubset
        if {'first_name', 'last_name', 'email', 'password', 'company', 'position'}.issubset(request.data):
            ...
            try:
                ...
                for item in password_error:  # подавляет предупреждение PyCharm при проверке кода
    ...
    def post(self, request, *args, **kwargs):
        ...
        if url:
            ...
            try:
                ...
                try:
                    update_price.delay(url=url, user_id=user_id)
                    return JsonResponse({'Status': True})
